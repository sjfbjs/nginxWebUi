			...
			for (Location location : locationList) {
				...
				if (location.getType() == 0 || location.getType() == 2) { // location或负载均衡
					...
					if (location.getHeader() == 1) { // 设置header
						ngxParam = new NgxParam();
						//判断locationParamJson存在Host就不加，如果存在就加
						//现在是默认就加
						if  (location.getProxyHost() != null){
							ngxParam.addValue("proxy_set_header " +  location.getProxyHost());
							ngxBlockLocation.addEntry(ngxParam);
						}else{
...
//						ngxBlockLocation.addEntry(ngxParam);
					...
					if (location.getRootType() != null ) {
						...
						//待定是否保留-----------------------
						if (location.getRootType().equals("tryroot")) {
							ngxParam = new NgxParam();
							ngxParam.addValue("root " + location.getRootPath()+ ";");
							ngxParam.addValue("\n") ;
							if (location.getTryFiles() != null ) {
								ngxParam.addValue("   try_files " + location.getTryFiles());
							}
							ngxBlockLocation.addEntry(ngxParam);
						}
						if (location.getRootType().equals("tryalias")) {
							ngxParam = new NgxParam();
							ngxParam.addValue("alias " + location.getRootPath()+ ";");
							ngxParam.addValue("\n") ;
							if (location.getTryFiles() != null ) {
						...
						//这个应该放到其他参数里面去
//						if (location.getAddHeaderAllowOrigin() != null){
//							ngxParam = new NgxParam();
//							ngxParam.addValue("add_header  " + location.getAddHeaderAllowOrigin());
//							ngxBlockLocation.addEntry(ngxParam);
//						}
//						if (location.getAddHeaderAllowCredentials() != null){
//							ngxParam = new NgxParam();
//							ngxParam.addValue("add_header   " + location.getAddHeaderAllowCredentials());
//							ngxBlockLocation.addEntry(ngxParam);
//						}
//						if (location.getAddHeaderAllowHeaders() != null){
//							ngxParam = new NgxParam();
//							ngxParam.addValue("add_header   " + location.getAddHeaderAllowHeaders());
//							ngxBlockLocation.addEntry(ngxParam);
//						}
//						if (location.getAddHeaderAllowMethods() != null){
//							ngxParam = new NgxParam();
//							ngxParam.addValue("add_header  " + location.getAddHeaderAllowMethods());
//							ngxBlockLocation.addEntry(ngxParam);
//						}
//						if (location.getAddHeaderMaxAge() != null){
//							ngxParam = new NgxParam();
//							ngxParam.addValue("add_header   " + location.getAddHeaderMaxAge());
//							ngxBlockLocation.addEntry(ngxParam);
//						}
					}
