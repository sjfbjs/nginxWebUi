$(function () {
    ...
    form.on('checkbox(header)', function (data) {
        console.log("proxyHost发生变化");
        console.log(data, $(data.elem).attr("lang"));
        checkHostChecked($(data.elem).attr("checked"), $(data.elem).attr("lang"));
    });
    form.on('select(type)', function (data) {
        console.log("type发生变化");
        checkType(data.value, $(data.elem).attr("lang"));
    });
    form.on('select(rootType)', function (data) {
        console.log("rootType发生变化"+  data);
        checkRoot("data:",data.value,"uuid:",$(data.elem).attr("lang"));
    });
...
function  checkRoot(rootType,uuid) {
    if(rootType == "tryroot" || rootType == "tryalias"){
        $("#" + uuid + " input[name='tryFiles']").show();
    }else {
        $("#" + uuid + " input[name='tryFiles']").hide();
    }
}
function  checkHostChecked(checked,uuid) {
    if(checked){
        $("#" + uuid + " input[name='proxyHost']").show();
    }else {
        $("#" + uuid + " input[name='proxyHost']").hide();
    }
}
...
function addOver() {
    ...
    $(".itemList").children().each(function () {
        ...
        location.proxyHost = $(this).find("input[name='proxyHost']").val();
        ...
        location.tryFiles = $(this).find("input[name='tryFiles']").val();
        ...
        // console.log("当前tryFiles值:" + location.tryFiles);
    })
        ...
        success: function (data) {
            if (data.success) {
                ...
                for (let i = 0; i < list.length; i++) {
                    ...
                    $("#" + uuid + " input[name='proxyHost']").val(location.proxyHost);
                    $("#" + uuid + " input[name='rootType']").val(location.rootType);
                    console.log(location.rootType);
                    if (location.rootType == "tryroot" || location.rootType == "tryalias") {
                        console.log("走到了tryroot tralias判断");
                        console.log(location);
                        $("#" + uuid + " input[name='tryFiles']").val(location.tryFiles);
                    }
                    ...
                    checkRoot(location.rootType , uuid);
                    checkHostChecked(uuid);
                }
				...
				<td>
					<span name="valueSpan">
						<div class="layui-inline">
							...
							<input type="text"  style="width: 240px;" name="proxyHost" id="proxyHost_${uuid}" class="layui-input long" value=""  placeholder="${serverStr.example}：Host $host">
						</div>
					</span>
					
					<span name="rootPathSpan">
						<div class="layui-inline" style="width: 150px;">
							<select name="rootType" lang='${uuid}' lay-filter="rootType" >
								<option value="root">${serverStr.rootModel}</option>
								<option value="alias">${serverStr.aliasModel}</option>
								<option value="tryroot">try_files root</option>
								<option value="tryalias">try_files alias</option>
							</select>
						</div>
						<div class="layui-inline" style="width: 150px;">
							<input type="text" name="tryFiles" id="tryFiles_${uuid}" class="layui-input" placeholder="${serverStr.example}：$uri $uri/ /index.html">
						</div>
						<div class="layui-inline" style="width: 150px;">
					...
					<span  name="headerSpan">
						<div class="layui-inline">
							<input type="checkbox" name="header" title="${serverStr.headerAddHost}" lay-skin="primary" lay-filter="header"  checked> 
						</div>
						<div class="layui-inline">
